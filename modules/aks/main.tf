...
resource "azuread_application" "aks_sp" {
  name                       = "${var.prefix}-${var.cluster_name}-aks"
...
resource "azurerm_kubernetes_cluster" "aks-cluster" {
  ...
  node_resource_group = "${var.prefix}-${var.cluster_name}-nodegroup"
  ...
  default_node_pool {
    ...
    tags =  {
      name  = "${var.prefix}-${var.cluster_name}-system-node",
      owner = "DevOps"
    }
  }
...
resource "azurerm_kubernetes_cluster_node_pool" "aks-cluster-np" {
  ...
  tags =  {
      Name  = "${var.prefix}-${var.cluster_name}-worker-node",
      owner = "DevOps"
    }
  lifecycle {
    ignore_changes = [tags,]
